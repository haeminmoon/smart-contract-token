...
contract('Token', ([creator, receiver, thirdParty]) => {
  const NAME = '';
  const SYMBOL = '';
  const DECIMALS = 18;
  const INITIAL_SUPPLY = 1000000000; // 1 Billion tokens
  const TOTAL_SUPPLY = new BigNumber(INITIAL_SUPPLY * 10 ** DECIMALS);
  const transferAmount = new BigNumber('100e18');  // 100 tokens
  const approveAmount = new BigNumber('10e18');  // 10 tokens


  beforeEach(async () => {
    this.token = await Token.new(NAME, SYMBOL, DECIMALS, INITIAL_SUPPLY, { from: creator });
  });
  ...
  describe('ERC20::Standard Token', () => {
    ...
    it('assigns the initial total supply to the creator', async () => {
      ...
      const logs = decodeLogs(receipt.logs, Token, this.token.address);
